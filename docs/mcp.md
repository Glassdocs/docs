...
Your client will show the server as connected straight away — the tool list is public, so it can be read before you sign in. The browser prompt appears the first time your agent actually *uses* a tool. Either run `/mcp` in your session and sign in there, or just ask for something and sign in when prompted. You approve the connection on a consent screen that names exactly which organizations' knowledge bases the client will reach.
