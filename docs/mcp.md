...
It removes the install step, and it relaxes the permission: the hosted server is intended for everyone on your GitHub organization, not only its admins.

Reaching further than that — a client or a colleague who reads your KB but has no GitHub account — is a separate and larger question. Reader access to a published KB is enforced by Cloudflare Access on *your* account, from the staff and client grants you set; Glassdocs writes those grants but never authenticates those readers itself, and has no identity for them. Extending MCP to them means giving Glassdocs a way to know who they are, which is a deliberate architectural decision rather than a follow-up. It is not currently planned.
