...
6. **Press "Set up & deploy"** and watch live status in the dashboard. On the first run, Cloudflare Access is created to gate the site, and the deploy only reports success once the gate is proven — either an unauthenticated request being redirected to the Access login, or, while a brand-new site is still propagating and serving nothing, the Access app and its policies confirmed over the Cloudflare API. See [Publishing](publishing.md) for the full verification pipeline.
