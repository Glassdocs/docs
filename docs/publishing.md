...
8. **Act on the verdict.** A proven-exposed site (content served to an unauthenticated request) is undone: production is rolled back to the previous successful build, or — when that run created the Pages project, so there is no earlier build to return to — the project is deleted. An unreachable site is *not* treated as exposed, since nothing has been served to anyone; the publisher confirms the Access app and its policies over the Cloudflare API instead, which doesn't wait on the site becoming live. If neither the probe nor the API can confirm a gate, the run fails and says so — the deployment is left alone, because nothing is being served that could be taken down.
